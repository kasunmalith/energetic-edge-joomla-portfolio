var RokAjaxSearch = new Class({
	...
	initialize : function(b) {
		...
		this.inputBox = document.id("roksearch_search_str").set("autocomplete", "off");
		var d = this.inputBox.getCoordinates();
		...
		if (this.options.websearch || this.options.blogsearch || this.options.imagesearch) {
			$$("#rokajaxsearch .search_options input[type=radio]").each(function(e) {
	...
	getLeft : function() {
		var d = this.inputBox.getCoordinates(), a = document.id("roksearch_results").getSize().x;
		var c = document.id(window).getSize(), b;
		if (c.x / 2 < d.left + d.width) {
			b = d.left + d.width - a;
		} else {
			b = d.left;
		}
	...
	addEvents : function() {
		this.inputBox.addEvents({
			...
			keyup : function(d) {
				if (d.code == 17 || d.code == 18 || d.code == 224 || d.alt || d.control || d.meta) {
					return false;
				}
				if (d.alt || d.control || d.meta || d.key == "esc" || d.key == "up" || d.key == "down" || d.key == "left" || d.key == "right") {
					return true;
				}
				if (d.key == "enter") {
					d.stop();
				}
				if (d.key == "enter" && this.selectedEl != -1) {
				...
				clearTimeout(this.timer);
				var g = this.options.searchlink.split("?")[0];
				g = g.replace(this.options.uribase, "");
				g = (g) ? g : "index.php";
				var c = this.options.uribase + g;
				if (this.options.wordpress) {
					c = this.options.uribase + this.options.searchlink;
				}
				if (this.inputBox.value == "") {
					...
					if (this.type == "local") {
						...
						var b = new Request({
							...
							onSuccess : function(o, p, m) {
								var i = new Element("div", {
									styles : {
										display : "none"
									}
								}).set("html", o);
								var k = document.id("rokajaxsearch_tmp");
								var e = i.getElement(".contentpaneopen");
								if (e) {
									i.getChildren().each(function(r) {
										if (r.get("class") == "contentpaneopen" && r.id != "page") {
											k.set("html", r.innerHTML);
										}
									});
								} else {
									i.inject(document.body);
									i.setStyles({
										display : "block",
										position : "absolute",
										top : -10000
									});
									e = i.getElement("div.search-results") || i.getElement("div.search") || i.getElement("div[id=page]");
									if (!e) {
										e = i.getElement("div.search");
									}
									i.dispose();
									if (e) {
										var h = e.getElement(".search-results") || e.getElement(".search") || e.getElement(".results");
										k.adopt(h);
									}
								}
								this.results.empty().removeClass("roksearch_results").setStyle("visibility", "visible");
								...
								this.selectedEl = -1;
								this.els = [];
								if (i.getElement(".contentpaneopen")) {
									this.outputTable();
								} else {
									this.outputTableless();
								}
								k.empty().setStyle("visibility", "visible");
								this.inputBox.removeClass("loading");
								var n = this.inputBox.getCoordinates(), q = this;
								this.results.setStyles({
									top : n.top + n.height,
									left : q.getLeft()
								});
	...
	outputTable : function() {
		...
		if (!g.length) {
			...
			if (this.options.includelink) {
				var s = $$("#rokajaxsearch input[name=limit]")[0];
	...
	outputTableless : function() {
		...
		this.results.addClass("roksearch_results");
		var n = $$("#rokajaxsearch_tmp ol.list li"), c, g, e;
		if (!n.length) {
			n = $$("#rokajaxsearch_tmp dl dt");
		}
		if (n.length > 0) {
			g = new Element("div", {
				"class" : "container-wrapper"
			...
			n.each(function(M, I) {
				...
				if (H.length > 0) {
					...
					var C = new Element("h3").set("html", H[0].get("text")).inject(K);
					this.els.push(F);
					F.addEvents({
						mouseenter : function() {
					...
					var P = "";
					if (this.options.showdescription) {
						var O = M.getNext(".result-text") || H[2];
						P = O.innerHTML;
					}
					var J = new Element("span").set("html", P).inject(K, "after"), Q;
					if (this.options.showcategory) {
						var L = M.getNext(".result-category") || H[1];
		...
		if (!n.length) {
			...
			if (this.options.includelink) {
				var x = $$("#rokajaxsearch input[name=limit]")[0];
				this.bottombar = new Element("div", {
					"class" : "roksearch_row_btm png"
				});
				var b = new Element("a", {
