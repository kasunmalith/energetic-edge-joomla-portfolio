...
function loadFskalbum(folderName,count,articleurl,article_id,server_id){

	$j1("#fsbutton").click();
	
send_foldername=folderName;
folderName=folderName+count;
var divName='<div id="fullscreenalbum" class="'+folderName+'_main fullscreenaalbum"><div class="album_content"><center> <img class="content-1-loading" src="/images/preloader-w8-cycle-white.gif"></center></div><div onclick="close_fullscreenaalbum(\x27'+folderName+'\x27)" id="'+folderName+'_close" class=" fullscreenaalbum-close"></div><div id="fsk-sidebar"></div></div>';
$j1("body").addClass('removeScroll');
$j1("body").append(divName); //body append

$j1("."+folderName+"_main .album_content").load('plugins/content/fskmalbum/load.php?count='+count+'&send_foldername='+send_foldername+'&articleurl='+articleurl+'&article_id='+article_id+'&server_id='+server_id,function(){
...
function deletePhoto(target,server_id){
	    target = target.replace("http://www", "www");
	$j1('.delete-img').load('plugins/content/fskmalbum/delete.php?target='+target+'&server_id='+server_id);
}
